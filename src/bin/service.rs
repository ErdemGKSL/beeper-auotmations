...
use beeper_auotmations::notifications::service::NotificationService;
use tokio::signal;
...
async fn main() -> Result<()> {
    ...
    let app_state = SharedAppState::new(config.clone());
    
    // Create hot reload channel
    let (reload_tx, reload_rx) = tokio::sync::mpsc::channel::<Config>(10);
    
    // Always start the service with the reload receiver
    let _notification_service = NotificationService::new(app_state.clone(), reload_rx);
...
fn print_config_status(config: &Config) {
